...
class _AnimatedBottomBar extends State<AnimatedBottomBar> with TickerProviderStateMixin {
  ...
  Widget _buildNormalButton(int index, double widgetScreen){
    ...
    new StreamBuilder(stream: _streamController.stream, initialData: 0, builder: (context, AsyncSnapshot<int> snapshot) {
      return
        new Container(width: widgetScreen, height:50, padding: EdgeInsets.symmetric(vertical: 10), child:
        new Icon(activated ? widget.buttonsHiddenIcons[index] : widget.buttonsIcons[index], size: snapshot.data == index && !activated ? 25 : 25, color: snapshot.data == index && !activated ? widget.activatedIconColor : widget.defaultIconColor)
        );
    })
    ), onTap: (){
  ...
  Widget _buildMiddleButton(double widgetScreen){
    return new Stack(alignment: Alignment.bottomCenter,children: <Widget>[
      new AnimatedOpacity(duration: Duration(milliseconds: 100),opacity: activated ? 0 : 1.0,
          child:
      new ClipPath(clipper: BottomWaveClipper(convex: _dropContainerPosition), child:
      new Container(color: widget.background, width: widgetScreen + 30, height: widgetScreen*0.9)
      )
      ),
